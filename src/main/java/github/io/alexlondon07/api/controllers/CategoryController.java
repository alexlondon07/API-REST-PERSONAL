...
public class CategoryController {
	...
	@GetMapping(value = "/categories", headers = Constants.JSON)
	public @ResponseBody ResponseEntity<List<Category>> getCategories(
			@RequestParam(value = "name", required = false) String name,
			@RequestParam(value = "ide_category", required = false) Long ideCategory) {
		...
		if (ideCategory != null && ideCategory > 0) {
			...
			if (category == null) {
				return new ResponseEntity(
						new CustomErrorType("Category with ide_category " + ideCategory + " not found ",
								MessageType.ERROR),
		...
		if (name != null) {
			...
			if (category == null) {
				return new ResponseEntity(
						new CustomErrorType("Category with name " + name + " not found ", MessageType.ERROR),
						HttpStatus.NOT_FOUND);
			} else {
				return new ResponseEntity(category, HttpStatus.OK);
			}
	...
	@PostMapping(value = "/categories", headers = Constants.JSON)
	public ResponseEntity<Category> createCategory(@Validated @RequestBody Category category,
			UriComponentsBuilder uriBuilder, BindingResult bindingResult) {
		...
		if (bindingResult.hasErrors()) {
			...
			if (categoryService.isCategoryExist(category)) {
				return new ResponseEntity(new CustomErrorType(
						"Unable to Create. A Category with name " + category.getName() + " already exist.",
			...
			headers.setLocation(uriBuilder.path(Constants.API_VERSION + Constants.CATEGORIES + "{id}")
					.buildAndExpand(category.getIdeCategory()).toUri());
	...
	public ResponseEntity<Category> updateCategory(@Validated @PathVariable("id") Long id,
			@RequestBody Category category, BindingResult bindingResult) {

		logger.info("Updating Category id {} ", id);

		if (id == null || id <= 0) {
			return new ResponseEntity(new CustomErrorType("idCategory is required", MessageType.ERROR),
					HttpStatus.CONFLICT);
		}
		...
		if (bindingResult.hasErrors()) {
			...
			if (currentCategory == null) {
				return new ResponseEntity(
						new CustomErrorType("Unable to update. A Category with id " + id + " not found.",
								MessageType.INFO),
						HttpStatus.CONFLICT);
			}
